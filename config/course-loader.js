(function() {
  ...
  if (lessonId) {
    ...
    VaultUtils.waitForFirebase(function() {
  ...
  function renderCourseIndex(container, courseId, courseConfig, structure, auth, db) {
    ...
    structure.chapters.forEach(chapter => {
      ...
      html += '<p class="course-chapter-title">' + VaultUtils.escapeHtml(chapter.title) + '</p>';
      ...
      chapter.lessons.forEach(lessonId => {
        ...
        html += '<div class="course-lesson-item" data-lesson="' + VaultUtils.escapeHtml(lessonId) + '">';
        html += '<div class="course-lesson-link">' + VaultUtils.escapeHtml(lessonTitle) + '</div>';
      ...
      Promise.all([
        ...
        const handleComplete = function() {
          ...
          if (showProgress && !completed.includes(lessonId)) {
            // Mark as complete using VaultProgress (centralized progress management)
