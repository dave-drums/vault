/* vault-create-account.js - Inline version
...
(function () {
  ...
  function renderForm({ token, email }) {
    root.innerHTML = `
      <div class="vca-wrap">
        <div class="vca-card">
          ...
          <div class="vca-field">
            <label>Email</label>
            <input id="vca-email" type="email" value="${VaultUtils.escapeHtml(email)}" readonly>
          </div>
          ...
          <div class="vca-field">
            <label class="vca-age-confirm-label">
              <input id="vca-age-confirm" type="checkbox" class="vca-age-checkbox">
              <span class="vca-age-confirm-text">I am above 16 years of age</span>
            </label>
          </div>
    ...
    btn.addEventListener("click", async () => {
      ...
      const ageConfirmed = document.getElementById("vca-age-confirm").checked;
      ...
      try {
        ...
        const deviceEmoji = deviceType === 'mobile' ? '📱' : '🖥️';
        
        batch.set(metricsRef, {
          createdAt: inviteCreatedAt,
          lastLoginAt: nowServer,
  ...
  function injectStyles() {
    const css = `
      ...
      .vca-field .vca-age-confirm-label{display:flex !important;flex-direction:row;align-items:center;gap:12px;margin:0;}
      .vca-field input[type="text"],
      .vca-field input[type="email"],
      .vca-field input[type="password"]{display:block;width:100%;box-sizing:border-box;padding:10px;border:1px solid #ccc;border-radius:6px;font-family:'Inter',sans-serif;font-size:var(--text-ui);}
      .vca-field input[type="checkbox"]{width:18px;height:18px;cursor:pointer;margin:0;}
      .vca-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;}
      @media (max-width:520px){.vca-grid{grid-template-columns:1fr;}}
      .vca-hint{margin-top:6px;font-size:var(--text-micro);opacity:.75;}
      .vca-age-confirm-text{font-family:'Inter',sans-serif;font-size:var(--text-small);color:var(--text-primary);user-select:none;}
